...
bacterias_rizosfera_ancom<-subset_samples(bacterias_rizosfera,id!="BF5B")
ancom_clase_bacterias_rizosfera<-ancombc(bacterias_rizosfera_ancom,assay_name = counts,tax_level = "Phylum",group = "Altitud",formula = "Altitud")
...
colnames(ancom_subset_bacterias_rizo)<-c("taxon","LFC_1978","LFC_2007","LFC_2178","LFC_2210","diff_1978","diff_2007","diff_2178","diff_2210")

ancom_subset_bacterias_rizo<-subset(ancom_subset_bacterias_rizo,diff_2007!=FALSE|diff_2178!=FALSE|diff_2210!=FALSE|diff_1978!=FALSE)
...
ancom_subset_bacterias_rizo<-gather(ancom_subset_bacterias_rizo,LFC_1978,LFC_2007,LFC_2178,LFC_2210,key="Elevation",value="LFC")

ancom_subset_bacterias_rizo<-subset(ancom_subset_bacterias_rizo,taxon!="Pezizomycotina_cls_Incertae_sedis")
ancom_plot_bacterias_rizo<-ggplot(data=ancom_subset_bacterias_rizo,aes(x=taxon,y=LFC,fill=taxon))+
  ...
  theme_biome_utils()+
  theme(legend.position = "right")+
  scale_fill_manual(values=moma.colors("Warhol",34),name="Phylum")+
  theme(axis.text.x = element_blank())+
  theme(axis.title.x = element_blank())+
  theme(legend.box.spacing = unit(0, "pt"))+
  ggtitle("a")+
