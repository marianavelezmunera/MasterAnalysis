...
total_bacterias<-microbiome::transform(aggregate_top_taxa2(subset_taxa(bacterias_rare_taxo,!is.na(Order)),"Order",top=15),"compositional")


plot_composition(total_bacterias,group_by = "Tipo_muestra",sample.sort = "Altitud")+
  theme_biome_utils()+
  theme(legend.position = "right")+
  scale_fill_manual(name="Orden",values=moma.colors("Warhol",21))+
  ylab("Abundance")+
  xlab("Sample")+
